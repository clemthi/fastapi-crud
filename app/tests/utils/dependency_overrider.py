    ...
    def __enter__(self):
        for dep, new_dep in self.overrides.items():
            if dep in self._app.dependency_overrides:
                self._old_overrides[dep] = self._app.dependency_overrides[dep]  # Save existing overrides
    ...
    def __exit__(self, *args: typing.Any) -> None:
        for dep in self.overrides.keys():
            if dep in self._old_overrides:
                self._app.dependency_overrides[dep] = self._old_overrides.pop(dep)  # Restore previous overrides
            else:
                del self._app.dependency_overrides[dep]  # delete the entry
